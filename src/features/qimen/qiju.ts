 ...
 * 九宫位置信息
 */
const JIU_GONG: Record<string, any> = {
 ...
 * 九星信息
 */
const JIU_XING: Record<string, any> = {
 ...
 * 八门信息
 */
const BA_MEN: Record<string, any> = {
 ...
 * 八神信息
 */
const BA_SHEN: Record<string, any> = {
...
export interface QiMenResult {
  ...
  dayXunShou: string;
  luoGongGan: string;
  sanQiLiuYi: Record<string, string>;
  ...
  anGan: Record<string, string>;
  zhiFuGong: string;
  zhiFuYuanGong: string;
  zhiFuXing: string;
  zhiShiGong: string;
  zhiShiMen: string;
  kongWangZhi: string[];
  kongWangGong: string[];
  maStar: { zhi: string; gong: string };
  jiuGongAnalysis: Record<string, GongAnalysis>;
...
function calculateJuShu(date: Date, method: string = '时家'): JuShuInfo {
  ...
  if (method === '时家') {
    ...
    ganZhi = lunar.getDayInGanZhi(); // 暂时使用日干支
  } else if (method === '年家') {
    ganZhi = lunar.getDayInGanZhi(); // 暂时使用日干支
  }
  ...
  // 判断阴阳遁（冬至到夏至前为阳遁，夏至到冬至前为阴遁）
  const solarYear = solar.getYear();
  const dongZhiDate = (Solar as any).fromYmd(solarYear - 1, 12, 22); // 去年的冬至，约12月22日
  const xiaZhiDate = (Solar as any).fromYmd(solarYear, 6, 21); // 今年的夏至，约6月21日
  const nextDongZhiDate = (Solar as any).fromYmd(solarYear, 12, 22); // 今年的冬至

  let yinYangType: 'yang' | 'yin';
  if ((date >= dongZhiDate && date < xiaZhiDate) || (date >= nextDongZhiDate)) {
    yinYangType = 'yang'; // 阳遁
  } else {
    yinYangType = 'yin'; // 阴遁
  }
 ...
 * @returns 旬首
 */
function getXunShou(lunar: any, method: string): string {
  let xun = '';
  let ganZhi = '';
  ...
  try {
    ...
    // 旬名与六仪的对应关系
    const xunToLiuYi: Record<string, string> = {
      '甲子': '戊', '甲戌': '己', '甲申': '庚',
      '甲午': '辛', '甲辰': '壬', '甲寅': '癸'
    };
 ...
 * @returns 空亡地支数组
 */
function getKongWang(lunar: any, method: string): string[] {
  try {
    let xun = '';
    if (method === '时家') {
      xun = lunar.getTimeXun();
 ...
 * @returns 空亡宫位数组
 */
function getKongWangGong(kongWangZhi: string[]): string[] {
  // 地支对应的宫位（奇门遁甲地支与九宫对应）
 ...
 * @returns 干支
 */
function getLuoGongGanZhi(lunar: any, method: string): string {
  try {
    if (method === '时家') {
      ...
      return lunar.getDayInGanZhi().substring(0, 1); // 暂时使用日干
    } else if (method === '年家') {
      return lunar.getDayInGanZhi().substring(0, 1); // 暂时使用日干
    }
  } catch (e) {
 ...
 * @param _baMen 八门分布（未使用，保留参数兼容）
 * @param _diPan 地盘干分布（未使用，保留参数兼容）
 * @param type 阴阳遁类型 'yang' 或 'yin'
 * @returns 暗干分布
 */
function calculateAnGan(_baMen: Record<string, string>, _diPan: Record<string, string>, type: 'yang' | 'yin'): Record<string, string> {
  // 三奇六仪顺序
  const SAN_QI_LIU_YI = ['戊', '己', '庚', '辛', '壬', '癸', '丁', '丙', '乙'];
  const anGan: Record<string, string> = {};
 ...
 * @returns 天盘干分布
 */
function distributeTianPan(diPan: Record<string, string>, xunShou: string, shiGan: string, type: 'yang' | 'yin'): Record<string, string> {
  const result: Record<string, string> = {};

  // 1. 找旬首六仪在地盘上的位置 = 值符宫
  let zhiFuGong: string | null = null;
  ...
  // 2. 找时干在地盘上的位置 = 时干落宫
  let shiGanGong: string | null = null;
  for (const gong in diPan) {
 ...
 * @returns 宫位分析结果
 */
function analyzeGong(gongNumber: number, jiuXing: Record<string, string>, baMen: Record<string, string>, baShen: Record<string, string>): GongAnalysis {
 ...
 * 生成宫位解释文字
 */
function generateGongExplanation(gongStr: string, _gongInfo: any, xing: string, xingInfo: any, men: string, menInfo: any, shen: string, shenInfo: any, jiXiong: string): string {
  const gongExplain: Record<string, string> = {
 ...
 * @returns 综合分析结果
 */
function overallAnalysis(jiuGongAnalysis: Record<string, GongAnalysis>, zhiFuGong: string, zhiShiGong: string, purpose: string): OverallAnalysis {
  // 值符、值使宫的吉凶
 ...
 * @returns 排盘结果
 */
export function calculate(date: Date, options: any = {}): QiMenResult {
  ...
  try {
    ...
    // 1. 计算地盘干（根据局数固定排布）
    const diPan = getDiPan(juShu.type, parseInt(juShu.number));
    ...
    // 分析九宫吉凶
    const jiuGongAnalysis: Record<string, GongAnalysis> = {};
    for (let i = 1; i <= 9; i++) {
    ...
    return {
      basicInfo: {
        date: `${solar.toString()} ${String((solar as any).getHour()).padStart(2, '0')}:${String((solar as any).getMinute()).padStart(2, '0')}:${String((solar as any).getSecond()).padStart(2, '0')}`,
    ...
    return {
      ...
      siZhu: { year: '', month: '', day: '', time: '' },
      juShu: { jieQiName: '', type: 'yang', number: '1', yuan: '', fullName: '', formatCode: '' },
      xunShou: '',
      dayXunShou: '',
      luoGongGan: '',
      sanQiLiuYi: {},
      ...
      maStar: { zhi: '', gong: '' },
      jiuGongAnalysis: {},
      analysis: { overallJiXiong: 'ping', overallJiXiongText: '平', bestGong: '', suggestions: [] }
    } as QiMenResult;
  }
}
