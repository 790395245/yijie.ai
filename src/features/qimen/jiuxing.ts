...
// 九星原位分布
const BASIC_XING: Record<string, string> = {
  '1': '天蓬',
  '8': '天任',
  '3': '天冲',
  '4': '天辅',
...
export interface JiuXingResult {
  zhiFuGong: string;          // 值符原位宫（旬首六仪在地盘的位置）
  zhiFuLuoGong?: string;      // 值符落宫（转动后的位置，即时干落宫）
  zhiFuXing: string;          // 值符星
  jiuXing: Record<string, string>;  // 九星分布
}
...
export function distributeJiuXing(
  ...
  // 5. 计算九星转动
  const jiuXing: Record<string, string> = {};
  // 天禽寄坤二宫，中宫显示与坤二宫相同的星
